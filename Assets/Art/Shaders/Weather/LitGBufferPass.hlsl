...
GBufferFragOutput LitGBufferPassFragment(Varyings input)
{
    ...
    float snowMask = GlobalSnowMask(input.normalWS.xyz);
    surfaceData.albedo = lerp(surfaceData.albedo, _GlobalSnowColor.rgb, snowMask);
    surfaceData.metallic = lerp(surfaceData.metallic, 0, snowMask);
    surfaceData.smoothness = lerp(surfaceData.smoothness, 0.1, snowMask);

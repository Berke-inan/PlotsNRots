...
[CreateAssetMenu(menuName = "Plots & Rots/Weather/Sky Atmosphere Profile")]
public sealed class SkyAtmosphereProfile : ScriptableObject
{
    [Header("Day palette")]
    [SerializeField] private Color dayZenith = new Color(.11f, .34f, .62f);
    [SerializeField] private Color dayMiddle = new Color(.32f, .59f, .80f);
    ...
    [SerializeField] private Color nightMiddle = new Color(.025f, .045f, .10f);
    [SerializeField] private Color nightHorizon = new Color(.09f, .12f, .20f);
    ...
    [SerializeField] private Color nightCloudLight = new Color(.14f, .20f, .30f);
    [SerializeField] private Color nightCloudShadow = new Color(.035f, .055f, .10f);
    [SerializeField] private Color sunNoon = new Color(1f, .95f, .84f);
    [SerializeField] private Color sunHorizon = new Color(1f, .51f, .24f);
    [SerializeField] private Color moonTint = new Color(.57f, .72f, 1f);

    [Header("Sky cloud layers")]
    [SerializeField] private Texture2D cloudMask;
    [Min(.1f), SerializeField] private float macroScale = .95f;
    [FormerlySerializedAs("detailScaleRatio"), Min(1f), SerializeField] private float lowerLayerScale = 1.72f;
    [Range(.2f, 2f), SerializeField] private float farLayerSpeed = .55f;
    [Range(1f, 3f), SerializeField] private float nearLayerSpeed = 1.25f;
    [SerializeField] private Vector2 windDirection = new Vector2(1f, .3f);
    [Min(0f), SerializeField] private float cloudSpeed = .008f;
    [Range(.01f, .25f), SerializeField] private float coverageSoftness = .075f;
    [Range(0f, 1f), SerializeField] private float breakupStrength = .16f;
    [Range(2, 8), SerializeField] private int cloudToneSteps = 4;
    [Range(0f, 2f), SerializeField] private float highlightStrength = .9f;
    [Range(0f, 1f), SerializeField] private float undersideDarkness = .62f;
    [Range(.04f, .4f), SerializeField] private float horizonFade = .18f;

    [Header("Celestial")]
    [Range(.1f, 3f), SerializeField] private float sunAngularRadius = .9f;
    [Range(.1f, 3f), SerializeField] private float moonAngularRadius = 1.1f;
    [Range(0f, 1f), SerializeField] private float sunHalo = .18f;
    [Range(0f, 1f), SerializeField] private float moonHalo = .06f;
    [Range(0f, 1f), SerializeField] private float starDensity = .018f;
    [Range(0f, 3f), SerializeField] private float starBrightness = .9f;
    ...
    public static float Daylight(float solarElevation) =>
        Mathf.SmoothStep(0f, 1f, Mathf.InverseLerp(-.16f, .22f, solarElevation));

    public static float Twilight(float solarElevation) =>
        1f - Mathf.SmoothStep(0f, 1f, Mathf.InverseLerp(.015f, .28f, Mathf.Abs(solarElevation)));

    public static float Stars(float solarElevation) =>
        1f - Mathf.SmoothStep(0f, 1f, Mathf.InverseLerp(-.2f, -.04f, solarElevation));
}

...
public class VehicleUI : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Scriptin her yerden ulaþýlabilir olmasýný saðlar
        if (Instance == null) Instance = this;
    }

    // Dýþarýdan araç deðiþtirmek için kullanacaðýmýz fonksiyon
    public void AraciDegistir(VehicleController yeniArac)
    {
        vehicleController = yeniArac;

        // Eski kadraný ekrandan temizle (Farklý araçlarýn farklý max hýzý olacaðý için)
        if (speedometerContainer != null && uiDocument != null)
        {
            uiDocument.rootVisualElement.Remove(speedometerContainer);
            speedometerContainer = null;
        }

        // Eðer araca bindiysek yeni kadraný çiz
    ...
    {
        if (uiDocument != null && uiDocument.rootVisualElement != null && vehicleController != null)
        {
            KadraniOlustur();
        }
    }

    private void KadraniOlustur()
    {
        if (speedometerContainer != null) return;
    ...
    private void Update()
    {
        // Eðer araca binili deðilse veya kadran henüz oluþturulmadýysa UI'ý gizle
        if (vehicleController == null || speedometerContainer == null)
        {
            if (speedometerContainer != null) speedometerContainer.style.display = DisplayStyle.None;
            return;
        }
